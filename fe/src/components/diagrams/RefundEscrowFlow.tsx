...
const nodes: Node[] = [
  ...
  { id: "s1", position: { x: 50, y: 60 }, data: { label: "refundEscrow(escrowId)" }, style: stepStyle, sourcePosition: Position.Right, targetPosition: Position.Left },
  ...
  { id: "s4", position: { x: 320, y: 180 }, data: { label: "transfer(escrow, depositor, amount)" }, style: stepStyle, sourcePosition: Position.Right, targetPosition: Position.Left },
...
const edgeStyle = { stroke: "var(--danger)", strokeWidth: 2 };
...
export function RefundEscrowFlow() {
  return (
    <div data-testid="diagram-refund-escrow-sequence">
      <h3 className="mb-2 text-sm font-semibold text-text-secondary">Refund Escrow Sequence</h3>
